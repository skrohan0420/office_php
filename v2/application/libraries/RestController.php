...
namespace instanthealth\RestServer;
 ...
 *
 * @link            https://github.com/instanthealth/codeigniter-restserver
 *
 * @version         4.0.0
 */
class RestController extends \CI_Controller
{
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        /////////////////////////////////////
        if ($this->config->item('rest_enable_platform')){
            $headers = $this->input->request_headers();
            if (!array_key_exists("platform",$headers)){               
                $this->response([
                    $this->config->item('rest_status_field_name')  => false,
                    $this->config->item('rest_message_field_name') => sprintf($this->lang->line('text_rest_invalid_platform')),
                ], self::HTTP_FORBIDDEN);
            }else if(array_key_exists("platform",$headers)){
                $allowed_platfomrs = $this->config->item('rest_allowed_platforms');
                $platform = $headers["platform"];
                $is_valid_platform = in_array($platform, $allowed_platfomrs);
                if(!$is_valid_platform){
                    $this->response([
                        $this->config->item('rest_status_field_name')  => false,
                        $this->config->item('rest_message_field_name') => sprintf($this->lang->line('text_rest_unknown_platform')),
                    ], self::HTTP_FORBIDDEN);
                }
            }
        }

        if ($this->config->item('rest_enable_deviceid')){
            $headers = $this->input->request_headers();
            if (!array_key_exists("deviceid",$headers)){                
                $this->response([
                    $this->config->item('rest_status_field_name')  => false,
                    $this->config->item('rest_message_field_name') => sprintf($this->lang->line('text_rest_invalid_deviceid')),
                ], self::HTTP_FORBIDDEN);
            }
        }
        /////////////////////////////////////
